...
class pci_generator:
    def __init__(self, N : int, alpha: float, beta : float, rho : float, sigma_M : float, sigma_R : float, sigma_X: float):
        self.N = N
        self.alpha = alpha
        self.beta = beta
        self.rho = rho
    ...
    def f_generate_par(self):
        ...
        M = arma_generate_sample(ar_M, ma, self.N, scale=self.sigma_M) # By default normal distribution
        R = arma_generate_sample(ar_R, ma, self.N, scale=self.sigma_R) # By default normal distribution
        S = M + R
        return S, M, R
    ...
    def f_generate_trajectories(self, trajectory : int = 100):
        simul = np.zeros((self.N, trajectory, 2))

        # simul[:, i, 0] = X1, simul[:, i, 1] = X2
        for i in range(trajectory):
            simul[:, i, 0], simul[:, i, 1], _, _, _ = self.f_generate_pci()
